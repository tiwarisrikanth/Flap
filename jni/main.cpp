...
#include "models/assets/PTModelAssetCharacter.h"
...
jstring Java_com_secrethq_utils_PTJniHelper_jsSettingsString(JNIEnv *env, jobject thiz) {
    return env->NewStringUTF(PTPSettingsController::jsSettingsString().c_str());
}

void Java_com_secrethq_utils_PTJniHelper_setSettingsValue(JNIEnv *env, jobject thiz, jstring path, jstring value) {
    PTPSettingsController::setSettingsValue(env->GetStringUTFChars(path, 0), env->GetStringUTFChars(value, 0));
}

jstring Java_com_secrethq_utils_PTJniHelper_getSettingsValue(JNIEnv *env, jobject thiz, jstring path) {
    return env->NewStringUTF(PTPSettingsController::getSettingsValue(env->GetStringUTFChars(path, 0)).c_str());
}

void Java_org_cocos2dx_lib_Cocos2dxRenderer_nativeInit(JNIEnv*  env, jobject thiz, jint w, jint h){
    cocos2d::log("cocos2dx - Native INIT");
