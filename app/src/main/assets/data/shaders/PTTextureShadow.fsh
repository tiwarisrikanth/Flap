...
void main() {
    ...
    if (hasTexture == 1) {
        gl_FragColor = texture2D(CC_Texture0, texCoord / textureScale + textureOffset) * diffuseColor * mix(vec4(1.0, 1.0, 1.0, 0.0), shade, vec4(inc, inc, inc, 1.0)); //default was 0.5//todo: need add color handler here
    }
    else {
        gl_FragColor = diffuseColor * mix(vec4(1.0, 1.0, 1.0, 0.0), shade, vec4(inc, inc, inc, 1.0)); //default was 0.5//todo: need add color handler here
    }
